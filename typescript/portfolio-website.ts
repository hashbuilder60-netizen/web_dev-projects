...
const cardsEl = document.getElementById("cards") as HTMLElement | null;
const filters = [...document.querySelectorAll(".filter")] as HTMLButtonElement[];
...
const modalDesc = document.getElementById("modal-desc") as HTMLElement | null;
const modalResult = document.getElementById("modal-result") as HTMLElement | null;
const leadForm = document.getElementById("lead-form") as HTMLFormElement | null;
const leadMsg = document.getElementById("lead-msg") as HTMLElement | null;
...
let filter: Project["type"] | "all" = "all";
let quoteIndex = 0;
...
filters.forEach((btn) => {
  btn.addEventListener("click", () => {
    ...
    filter = btn.dataset.filter as Project["type"] | "all";
    render();
  });
});
...
leadForm?.addEventListener("submit", (e) => {
  e.preventDefault();
  const name = (document.getElementById("lead-name") as HTMLInputElement | null)?.value.trim() ?? "";
  const email = (document.getElementById("lead-email") as HTMLInputElement | null)?.value.trim() ?? "";
  const budget = (document.getElementById("lead-budget") as HTMLSelectElement | null)?.value ?? "";

  if (!name || !email || !budget) {
    if (leadMsg) leadMsg.textContent = "Please complete all inquiry fields.";
    return;
  }
  if (!EMAIL_RE.test(email)) {
    if (leadMsg) leadMsg.textContent = "Please provide a valid email address.";
    return;
  }

  if (leadMsg) leadMsg.textContent = `Thanks ${name}. Inquiry received for ${budget}. I will reach out at ${email}.`;
  leadForm?.reset();
});
...
function render() {
  ...
  rows.forEach((p) => {
    ...
    button?.addEventListener("click", () => {
      ...
      if (modalDesc) modalDesc.textContent = p.summary;
...
function startQuoteRotation() {
  ...
  quoteTimer = window.setInterval(() => {
    ...
    if (quote) quote.textContent = quotes[quoteIndex].q;
    if (author) author.textContent = quotes[quoteIndex].a;
  }, 5000);
}
