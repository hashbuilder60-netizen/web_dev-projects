...
const current = document.getElementById("current") as HTMLElement | null;
const hourly = document.getElementById("hourly") as HTMLElement | null;
const forecast = document.getElementById("forecast") as HTMLElement | null;
const useLocationBtn = document.getElementById("use-location") as HTMLButtonElement | null;
...
if (f && city && unit && current && hourly && forecast && useLocationBtn) {
  ...
  useLocationBtn.addEventListener("click", () => {
    ...
    navigator.geolocation.getCurrentPosition(
      (pos) => void fetchByCoords(pos.coords.latitude, pos.coords.longitude, "Your location"),
...
async function fetchByCity(name: string) {
  if (!name) return;
  try {
    setLoading("Loading city...");
    const geo = await fetchJson<{ results?: GeoResult[] }>(
...
async function fetchByCoords(lat: number, lon: number, place: string) {
  try {
    ...
    const data = await fetchJson<WeatherResponse>(
      `https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lon}&current=temperature_2m,apparent_temperature,wind_speed_10m,relative_humidity_2m&hourly=temperature_2m,precipitation_probability&daily=temperature_2m_max,temperature_2m_min&timezone=auto&temperature_unit=${tempUnit}&wind_speed_unit=${windUnit}`
    );

    if (!data?.current || !data?.hourly || !data?.daily) throw new Error("Invalid payload");
...
function renderCurrent(data: WeatherResponse, place: string) {
  if (!current) return;
...
function renderHourly(data: WeatherResponse) {
  if (!hourly) return;
  const symbol = unit?.checked ? "F" : "C";
...
function renderForecast(data: WeatherResponse) {
  if (!forecast) return;
  const symbol = unit?.checked ? "F" : "C";
...
function setLoading(message: string) {
  ...
  if (current) current.innerHTML = `<p>${message}</p>`;
}
...
function setLoadingState(isLoading: boolean) {
  const submit = f?.querySelector("button[type='submit']") as HTMLButtonElement | null;
  if (submit) submit.disabled = isLoading;
  if (useLocationBtn) useLocationBtn.disabled = isLoading;
}
