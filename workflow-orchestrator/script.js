class WorkflowOrchestrator {
  constructor() {
    ...
    this.elements = {
      ...
      owner: document.getElementById("step-owner"),
      status: document.getElementById("step-status"),
      eta: document.getElementById("step-eta"),
      deleteButton: document.getElementById("delete-step"),
      simulateRun: document.getElementById("simulate-run"),
  ...
  bind() {
    ...
    this.elements.form.addEventListener("submit", (event) => {
      ...
      this.steps = this.steps.map((step) =>
        step.id === this.selectedId
          ? {
              ...step,
              title: this.elements.title.value.trim() || step.title,
      ...
      this.log(`Updated step "${this.elements.title.value.trim() || "Untitled"}".`);
      this.persist();
      this.render();
    });
    ...
    this.elements.deleteButton.addEventListener("click", () => {
      ...
      if (removed) this.log(`Removed step "${removed.title}".`);
      this.persist();
      this.render();
    });
  ...
  render() {
    ...
    this.elements.workflowList.querySelectorAll(".select-step").forEach((button) => {
      button.addEventListener("click", () => this.selectStep(button.dataset.id));
    });
    this.elements.workflowList.querySelectorAll(".move-up").forEach((button) => {
      button.addEventListener("click", () => this.moveStep(button.dataset.id, -1));
    });
    this.elements.workflowList.querySelectorAll(".move-down").forEach((button) => {
      button.addEventListener("click", () => this.moveStep(button.dataset.id, 1));
    });
  ...
  importJson() {
    try {
      ...
      if (!Array.isArray(payload.steps)) throw new Error("Invalid import payload.");
      this.commitHistory();
      this.steps = payload.steps.map((step) =>
        this.createStep(step.title || "Imported step", step.owner || "Unknown", step.status || "queued", step.eta || "n/a")
      );
      this.selectedId = this.steps[0]?.id || null;
      this.persist();
  ...
  log(message) {
    this.logs.unshift({ message, time: new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }) });
  }
